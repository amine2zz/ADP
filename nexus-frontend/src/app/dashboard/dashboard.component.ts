...
@Component({
      ...
      <div class="tab-content" style="margin-top: 2rem;">
        ...
        <div *ngIf="currentTab === 'overview'" class="fade-in">
          <div class="kpi-grid">
            ...
            <div class="kpi-card" style="border-top: 4px solid #f59e0b; cursor: pointer;" (click)="setTab('advances')">
              <div class="kpi-title">Pending Advances</div>
              <div class="kpi-value">{{ advanceSummary.pending }}</div>
              <div class="kpi-trend" [class.negative]="advanceSummary.pending > 0">{{ advanceSummary.pending > 0 ? 'Awaiting Review ↗' : 'All Reviewed' }}</div>
            </div>
          </div>
        ...
        <div *ngIf="currentTab === 'advances'" class="fade-in">

          <!-- Summary KPIs -->
          <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
            <div class="kpi-card" style="border-top: 4px solid #f59e0b; text-align: center;">
              <div class="kpi-title">Pending</div>
              <div class="kpi-value" style="font-size: 2rem; color: #b45309;">{{ advanceSummary.pending }}</div>
            </div>
            <div class="kpi-card" style="border-top: 4px solid #137333; text-align: center;">
              <div class="kpi-title">Approved</div>
              <div class="kpi-value" style="font-size: 2rem; color: #137333;">{{ advanceSummary.approved }}</div>
            </div>
            <div class="kpi-card" style="border-top: 4px solid #ef4444; text-align: center;">
              <div class="kpi-title">Rejected</div>
              <div class="kpi-value" style="font-size: 2rem; color: #ef4444;">{{ advanceSummary.rejected }}</div>
            </div>
            <div class="kpi-card" style="border-top: 4px solid #6366f1; text-align: center;">
              <div class="kpi-title">Total Requests</div>
              <div class="kpi-value" style="font-size: 2rem; color: #4338ca;">{{ advanceSummary.total }}</div>
            </div>
          </div>
          ...
          <div class="card table-card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 0.75rem;">
              <h3 style="color: var(--adp-charcoal);">Salary Advance Requests</h3>
              <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
                <div style="display: flex; border: 1px solid var(--adp-border); border-radius: 6px; overflow: hidden;">
                  <button *ngFor="let s of ['ALL','PENDING','APPROVED','REJECTED']"
                          (click)="advanceStatusFilter = s"
                          [style.background]="advanceStatusFilter === s ? 'var(--adp-red)' : 'white'"
                          [style.color]="advanceStatusFilter === s ? 'white' : 'var(--adp-charcoal)'"
                          style="padding: 0.4rem 0.75rem; border: none; cursor: pointer; font-size: 0.8rem; font-weight: 600;">
                    {{ s }}
                  </button>
                </div>
                <input type="text" [(ngModel)]="advanceSearch" placeholder="Search employee..."
                       style="padding: 0.4rem 0.75rem; border: 1.5px solid var(--adp-border); border-radius: 6px; font-size: 0.85rem;">
              </div>
            </div>

            <div class="table-container" style="overflow-x: auto;">
              <table class="adp-table" *ngIf="filteredAdvances.length > 0; else noAdvances">
                <thead>
                  <tr>
                    <th>Employee</th>
                    <th>Amount</th>
                ...
                <tbody>
                  <tr *ngFor="let adv of filteredAdvances">
                    <td>
                      <strong>{{ adv.employee?.firstName }} {{ adv.employee?.lastName }}</strong>
                      <div style="font-size: 0.72rem; color: #64748b;">{{ adv.employee?.department?.name || 'No Dept' }}</div>
                    </td>
                    <td style="font-weight: 700; color: var(--adp-charcoal); white-space: nowrap;">{{ adv.amount | number:'1.2-2' }} TND</td>
                    <td style="color: #64748b; font-size: 0.82rem; white-space: nowrap;">{{ adv.requestDate }}</td>
                    <td style="max-width: 200px; font-size: 0.82rem; color: #475569;">{{ adv.reason || '—' }}</td>
                    <td>
                      <span [ngClass]="adv.status === 'PENDING' ? 'adv-badge adv-pending' : adv.status === 'APPROVED' ? 'adv-badge adv-approved' : 'adv-badge adv-rejected'">
                        {{ adv.status }}
                      </span>
                    </td>
                    <td style="font-size: 0.8rem;">
                      <ng-container *ngIf="adv.reviewDate">
                        <div style="color: #64748b;">{{ adv.reviewDate }}</div>
                        <div *ngIf="adv.reviewNotes" style="color: #475569; font-style: italic; margin-top: 2px; font-size: 0.75rem;">{{ adv.reviewNotes }}</div>
                        <div *ngIf="adv.reviewedBy" style="color: #94a3b8; font-size: 0.7rem; margin-top: 1px;">
                          by {{ adv.reviewedBy.firstName }} {{ adv.reviewedBy.lastName }}
                        </div>
                      </ng-container>
                      <span *ngIf="!adv.reviewDate" style="color: #94a3b8;">—</span>
                    </td>
                    <td>
                      <div *ngIf="adv.status === 'PENDING'" style="display: flex; flex-direction: column; gap: 0.4rem;">
                        <input type="text" [(ngModel)]="advanceNotes[adv.id]" placeholder="Optional notes..."
                               style="padding: 0.3rem 0.5rem; border: 1px solid var(--adp-border); border-radius: 4px; font-size: 0.78rem; width: 100%; box-sizing: border-box;">
                        <div style="display: flex; gap: 0.4rem;">
                          <button (click)="approveAdvance(adv.id)"
                                  style="flex: 1; padding: 0.35rem; background: #137333; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.78rem; font-weight: 700;">
                            ✓ Approve
                          </button>
                          <button (click)="rejectAdvance(adv.id)"
                                  style="flex: 1; padding: 0.35rem; background: #ef4444; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.78rem; font-weight: 700;">
                            ✕ Reject
                          </button>
                        </div>
                      </div>
                      <span *ngIf="adv.status !== 'PENDING'" style="color: #94a3b8; font-size: 0.8rem;">Reviewed</span>
                    </td>
                  </tr>
                </tbody>
              </table>
              <ng-template #noAdvances>
                <p style="color: var(--adp-dark-gray); padding: 2.5rem; text-align: center; font-size: 0.9rem;">
                  {{ advanceStatusFilter === 'PENDING' ? 'No pending salary advance requests.' : 'No salary advance requests found.' }}
                </p>
              </ng-template>
            </div>
          </div>
  ...
  styles: [`
    ...
    .modal-overlay {
      ...
      padding: 1.5rem 0.75rem; overflow-y: auto; z-index: 1000;
    }
    .modal-card {
      background: white; border-radius: 1rem; padding: 1.75rem;
      width: min(90vw, 520px); max-height: calc(100vh - 3rem);
    ...
    .adv-badge { padding: 0.2rem 0.55rem; border-radius: 10px; font-size: 0.7rem; font-weight: 700; white-space: nowrap; }
    .adv-pending  { background: #fef3c7; color: #92400e; }
    .adv-approved { background: #dcfce7; color: #166534; }
    .adv-rejected { background: #fee2e2; color: #991b1b; }
...
export class MainDashboardComponent implements OnInit {
  ...
  // Salary Advances
  advances: any[] = [];
  advanceSummary: any = { pending: 0, approved: 0, rejected: 0, total: 0 };
  advanceSearch = '';
  ...
  fetchData() {
    this.http.get<any[]>('http://localhost:8085/api/employees').subscribe({ next: (data) => this.allEmployees = data, error: (err) => console.error(err) });
    this.http.get<any[]>('http://localhost:8085/api/hr/all-leaves').subscribe({ next: (data) => this.leaves = data, error: (err) => console.error(err) });
    this.http.get<any[]>('http://localhost:8085/api/hr/all-attendance').subscribe({ next: (data) => this.attendanceData = data, error: (err) => console.error(err) });
    this.http.get<any[]>('http://localhost:8085/api/hr/categories').subscribe({ next: (data) => this.categories = data, error: (err) => console.error(err) });
  }
  ...
  saveAttendanceEdit() {
    ...
    request.subscribe({
      next: () => { this.notifService.show('Attendance saved successfully.', 'success'); this.attendanceEditModalVisible = false; this.fetchData(); },
  ...
  getDayColor(empId: number, dateStr: string) {
    ...
    if (s.status === 'leave') { if (s.type === 'SICK') return '#ef4444'; if (s.type === 'UNPAID') return '#f59e0b'; return '#1967D2'; }
  ...
  getDayLabel(empId: number, dateStr: string) {
    ...
    const s = this.getDayStats(empId, dateStr);
    if (s.status === 'leave') { if (s.type === 'UNPAID') return 'UNPAID 💼'; return s.type; }
  ...
  // ── Salary Advances ──────────────────────────────────────────────────────
  loadAdvances() {
    this.http.get<any[]>('http://localhost:8085/api/advances/all').subscribe({
      next: (data) => { this.advances = data; this.advanceNotes = {}; },
      error: (err) => console.error('Advances load failed', err)
    });
    this.http.get<any>('http://localhost:8085/api/advances/summary').subscribe({
      next: (data) => this.advanceSummary = data,
      error: (err) => console.error('Advances summary failed', err)
    });
  }

  approveAdvance(id: number) {
    const notes = this.advanceNotes[String(id)] || '';
    this.http.put(`http://localhost:8085/api/advances/${id}/approve`, { notes, reviewerEmail: this.getReviewerEmail() }).subscribe({
      next: () => { this.notifService.show('Advance approved.', 'success'); this.loadAdvances(); },
      error: () => this.notifService.show('Failed to approve advance.', 'error')
    });
  }

  rejectAdvance(id: number) {
    const notes = this.advanceNotes[String(id)] || 'Rejected by HR';
    this.http.put(`http://localhost:8085/api/advances/${id}/reject`, { notes, reviewerEmail: this.getReviewerEmail() }).subscribe({
      next: () => { this.notifService.show('Advance rejected.', 'success'); this.loadAdvances(); },
      error: () => this.notifService.show('Failed to reject advance.', 'error')
    });
  }

  getReviewerEmail(): string {
    try {
      const stored = localStorage.getItem('adp_user_full');
      if (stored) { const u = JSON.parse(stored); return u.email || ''; }
    } catch { /* ignore */ }
  ...
  get filteredAdvances() {
    return this.advances.filter(a => {
      ...
      const matchSearch = !this.advanceSearch || name.includes(this.advanceSearch.toLowerCase());
      const matchStatus = this.advanceStatusFilter === 'ALL' || a.status === this.advanceStatusFilter;
  ...
  onAnnualChange() {
    ...
    this.policyForm.monthlyIncrement = annual > 0 ? Number((annual / 12).toFixed(2)) : 0;
  }

  editPolicy(cat: any) { this.selectedCategory = cat; this.policyForm = { ...cat }; }

  resetPolicyForm() { this.selectedCategory = null; this.policyForm = { name: '', annualLeaveAllowance: null, monthlyIncrement: null, description: '' }; }
