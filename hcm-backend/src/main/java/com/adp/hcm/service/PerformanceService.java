...
public class PerformanceService {
    ...
    @Autowired
    private PerformanceQuestionRepository questionRepository;
    ...
    public List<PerformanceEvaluation> launchMonthlyEvaluation(String period) {
        ...
        for (Employee emp : allEmployees) {
            if (emp.getManager() != null) {
                if (evaluationRepository.findByEmployeeAndPeriod(emp, period).isEmpty()) {
    ...
    public PerformanceEvaluation submitEvaluation(Long evalId, List<EvaluationResponse> responses, boolean isAdmin) {
        ...
        if (!isAdmin && "COMPLETED".equals(eval.getStatus())) {
            throw new RuntimeException("Evaluation is already completed and cannot be modified by a manager.");
        }
    ...
    public List<PerformanceReportDTO> generateReports(Long deptId, String period, Long managerId) {
        ...
        if (managerId != null) {
            allEmployees = employeeRepository.findByManagerId(managerId);
        } else {
        ...
        for (Employee emp : allEmployees) {
            // Skip the employee itself if they are HR_ADMIN and have no manager
    ...
    public PerformanceReportDTO mapToDTO(PerformanceEvaluation eval) {
        ...
        if (!eval.getResponses().isEmpty()) {
            double avg = eval.getResponses().stream()
        ...
        dto.setResponses(eval.getResponses().stream().map(r -> {
            ...
            if (r.getQuestion() != null) {
                qDTO.setQuestionText(r.getQuestion().getText());
            }
            if (r.getRating() != null) {
                qDTO.setRating(r.getRating());
                qDTO.setRatingValue(r.getRating().getValue());
            }
            return qDTO;
        }).collect(Collectors.toList()));
    ...
    private Double calculateWorkedHours(Employee emp, String period) {
        try {
            ...
            List<Attendance> attendances = attendanceRepository.findByEmployeeAndWorkDateBetween(emp, start, end);
            ...
            for (Attendance a : attendances) {
                if (a.getMorningIn() != null && a.getEveningOut() != null) {
                    ...
                    if (a.getLunchOut() != null && a.getAfternoonIn() != null) {
                        // Two-session day
                        long session1 = Duration.between(a.getMorningIn(), a.getLunchOut()).toMinutes();
                        long session2 = Duration.between(a.getAfternoonIn(), a.getEveningOut()).toMinutes();
                        totalMins = session1 + session2;
                    } else {
                        // Continuous day
                        totalMins = Duration.between(a.getMorningIn(), a.getEveningOut()).toMinutes();
                        // Optional: subtract 30-60 mins for lunch if session is long? 
                        // For now, let's stick to actual punches.
                    }
